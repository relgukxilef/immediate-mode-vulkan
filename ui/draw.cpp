...
dynamic_image::dynamic_image(renderer_data& ui, unsigned size) {
    {
...
void wait_frame(renderer* renderer) {
    ...
    if (!view.swapchain) {
...

        {
            ...
            check(vkCreateSwapchainKHR(
                r.device.get(), &create_info, nullptr, out_ptr(view.swapchain)
            ));
        }
        ...
        view.swapchain_images = std::make_unique<VkImage[]>(view.image_count);
        view.images = std::make_unique<image[]>(view.image_count);
    ...
    ::image& image = view.images[view.image_index];
...
bool draw(const draw_info& info) {
    renderer_data& ui = *get(info.renderer).d;
    auto& view = ui.view;
    if (!view.images)
        return false;
...
void submit(renderer* renderer) {
    renderer_data& ui = *get(renderer).d;
    auto& view = ui.view;
    if (!view.images)
        return;
    ::image& image = view.images[view.image_index];
    ...
    VkResult result = vkQueuePresentKHR(ui.present_queue, &present_info);
    if (result == VK_SUBOPTIMAL_KHR || result == VK_ERROR_OUT_OF_DATE_KHR) {
        std::exchange(view, {});
        return;
    }
